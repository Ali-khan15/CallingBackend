...
const vonage = new Vonage({
  ...
  applicationId: '2201a040-54e2-471b-abb1-dff778bcb4e3', // Add the application ID
});
...
app.post('/call-users', (req, res) => {
  ...
  numbers.forEach((number) => {
    vonage.voice.createOutboundCall({
      ...
      from: { type: 'phone', number: '+923355504440' }, // Your registered Vonage number
      ncco: [
        {
          action: 'talk',
          text: script,
      ...
      if (err) {
        ...
        return res.status(500).send({ message: 'Error calling user', error: err });
      } else {
        console.log('Vonage Response:', responseData);
        res.status(200).send({ message: 'Call initiated', data: responseData });
      }
    });
  });
